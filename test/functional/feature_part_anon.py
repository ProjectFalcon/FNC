...
class AnonTest(ParticlTestFramework):
    ...
    def run_test(self):
        ...
        txnHashes.append(nodes[0].sendfalcontoanon(sxAddrTo1_1, 1, '', '', False, 'node0 -> node1 p->a'))
        txnHashes.append(nodes[0].sendfalcontoblind(sxAddrTo0_1, 1000, '', '', False, 'node0 -> node0 p->b'))
        ...
        for k in range(5):
            txnHash = nodes[0].sendfalcontoanon(sxAddrTo1_1, 10, '', '', False, 'node0 -> node1 p->a')
        ...
        ro = nodes[1].sendtypeto('anon', 'falcon', outputs, 'comment_to', 'comment_from', 4, 32, True)
        assert(ro['bytes'] > 0)

        txnHashes.append(nodes[1].sendtypeto('anon', 'falcon', outputs))
